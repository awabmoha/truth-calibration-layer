...
def extract_answers(answer_obj: dict) -> list[str]:
    answers = []
    seen = set()
    for answer in answer_obj.get("text", []):
...
def main():
    ...
    parser.add_argument("--out-dir", default=str(ROOT / "data" / "benchmarks" / "squad"))
    ...
    base_name = f"squad_{args.split}_{len(rows)}"
    ...
    for item in rows:
        ...
        row = item["row"]
        item_id = f"squad-{args.split}-{row_idx}"
    ...
    with questions_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["id", "title", "context", "question", "accepted_answers"])
        writer.writeheader()
    ...
    with manual_review_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=[
                "id", "title", "context", "question", "accepted_answers", "model_answer",
    ...
    metadata = {
        ...
        "claim_boundary": "This prepares context-grounded benchmark input data only; it does not run TCL-v0 or produce model results.",
    }
